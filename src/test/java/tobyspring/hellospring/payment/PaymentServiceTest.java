...
class PaymentServiceTest {
    ...
    @BeforeEach
    void beforeEach() {
         this.clock = Clock.fixed(Instant.now(), ZoneId.systemDefault());Clock clock = Clock.fixed(Instant.now(), ZoneId.systemDefault());
    }
    ...
    private void testAmount(BigDecimal exRate, BigDecimal convertedAmount, Clock clock) throws IOException {
        PaymentService paymentService = new PaymentService(new ExRateProviderStub(valueOf(500)),clock);
